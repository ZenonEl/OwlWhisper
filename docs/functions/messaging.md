...
### **`Send(peerID, data, dataLength)`**

**Описание:** Универсальная функция отправки данных конкретному пиру. Автоматически создает соединение и стрим если их нет.
...
1. **Автоматически проверяется** существование соединения с пиром
2. **Если соединения нет** - автоматически создается через Connect()
3. **Автоматически создается стрим** если его нет
4. **Данные отправляются** через libp2p поток
5. **Возвращается результат** операции

**Важные замечания:**
- ✅ **Автоматическое создание соединения** - не нужно предварительно подключаться
- ✅ **Автоматическое создание стрима** - не нужно вызывать CreateStream
- ✅ **Умная функция** - сама разбирается с техническими деталями
- 💡 **Используйте для любых типов данных** - текст, файлы, структуры

---

> **Примечание:** Функция `CreateStream` не нужна! Функция `Send()` автоматически создает соединение и стрим при необходимости.
