...
func (x *Envelope) GetPing() *PingMessage {
	if x != nil {
...
func (x *Envelope) GetPong() *PongMessage {
	if x != nil {
...
type Envelope_ProfileInfo struct {
	ProfileInfo *ProfileInfo `protobuf:"bytes,8,opt,name=profile_info,json=profileInfo,proto3,oneof"` // Информация о профиле
}

type Envelope_Ping struct {
	Ping *PingMessage `protobuf:"bytes,9,opt,name=ping,proto3,oneof"` // Ping для проверки соединения
}

type Envelope_Pong struct {
	Pong *PongMessage `protobuf:"bytes,10,opt,name=pong,proto3,oneof"` // Pong в ответ на Ping
}
...
type ProfileInfo struct {
	...
	Metadata      map[string]string      `protobuf:"bytes,5,rep,name=metadata,proto3" json:"metadata,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"` // Например, {"clientVersion": "0.1.0", "platform": "desktop"}
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type PongMessage struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	RequestTimestamp int64                  `protobuf:"varint,1,opt,name=request_timestamp,json=requestTimestamp,proto3" json:"request_timestamp,omitempty"` // Возвращаем timestamp из соответствующего Ping
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}
...
func (x *PongMessage) GetRequestTimestamp() int64 {
	if x != nil {
...
const file_internal_protocol_chat_proto_rawDesc = "" +
	"\n" +
	"\x1cinternal/protocol/chat.proto\x12\bprotocol\"\xf3\x03\n" +
	"\bEnvelope\x12\x1d\n" +
	"\n" +
	...
	"\fprofile_info\x18\b \x01(\v2\x15.protocol.ProfileInfoH\x00R\vprofileInfo\x12+\n" +
	"\x04ping\x18\t \x01(\v2\x15.protocol.PingMessageH\x00R\x04ping\x12+\n" +
	"\x04pong\x18\n" +
	" \x01(\v2\x15.protocol.PongMessageH\x00R\x04pong\"\"\n" +
	"\bChatType\x12\v\n" +
	...
	"\vPongMessage\x12+\n" +
	"\x11request_timestamp\x18\x01 \x01(\x03R\x10requestTimestampB7Z5github.com/zentfany/owlwhisper-core/internal/protocolb\x06proto3"
...
var file_internal_protocol_chat_proto_msgTypes = make([]protoimpl.MessageInfo, 9)
var file_internal_protocol_chat_proto_goTypes = []any{
	...
	(*PingMessage)(nil),    // 7: protocol.PingMessage
	(*PongMessage)(nil),    // 8: protocol.PongMessage
...
var file_internal_protocol_chat_proto_depIdxs = []int32{
	...
	7, // 4: protocol.Envelope.ping:type_name -> protocol.PingMessage
	8, // 5: protocol.Envelope.pong:type_name -> protocol.PongMessage
